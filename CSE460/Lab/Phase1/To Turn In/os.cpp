...
int main(int argc, char *argv[])
{
  ...
  string assenblyFile = argv[1];
  int pos = assemblyFile.find(".");
  if (pos > asemblyFile.length() || assemblyFile.substr(pos) != ".s"{
      cout << "No .s suffex.\n";
      exit(2);
    }

    string name = assemblyFile.substr(0, pos);
    string objectFile = name + ".0";

    fstream assebly, objectCode;
